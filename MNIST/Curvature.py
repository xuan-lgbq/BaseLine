...
def curvature_dominant_projection(eigenvalues, top_eigenvectors, prev_eigenvector, top_k, device = device):
    """
    构造矩阵 P_k = Σ λ_i u_i u_i^T
    Args:
        eigenvalues: shape (k,) 前 k 个特征值 (torch tensor)
        top_eigenvectors: shape (p, k) 特征向量矩阵，每列为一个特征向量 (torch tensor)
        top_k: 使用前 k 个特征对构建矩阵
    Returns:
        Projection : the projection of previous eigenvector into P_k (torch tensor)
    """
    ...
    projection = torch.zeros_like(prev_eigenvector, device=device)
    for i in range(top_k):
        lambda_i = eigenvalues[i]
        u_i = top_eigenvectors[:, i].reshape(-1, 1)
        projection += lambda_i * torch.matmul(u_i, torch.matmul(u_i.T, prev_eigenvector))
...
def curvature_projection_trajectory(recorded_steps_top_eigenvectors, recorded_steps_eigenvalues, top_k):
    ...
    for i in range(1, num_steps):
        ...
        # 构造 P_k
        projection = curvature_dominant_projection(lambdas, eigvecs, v, top_k, device)
        
        # 计算 projection 的 2 范数
        norm_projection = torch.linalg.norm(projection)

        # 计算 v 的 2 范数
        norm_v = torch.linalg.norm(v)
        
        curvature = norm_projection / (norm_v + 1e-10)  # 防止除以0
