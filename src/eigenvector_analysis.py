...
def save_eigenvector_data(analysis_data, save_dir, step, method, lr, seed, rank, num_layer):
    """保存特征向量相关数据"""
    if analysis_data is None:
        return
    
    # 创建保存文件名
    base_filename = f"eigenvectors_step_{step}_{method}_lr{lr}_seed{seed}_rank{rank}_layer{num_layer}"
    ...
    np.savez(numpy_save_path,
             ...
             dominant_dim=analysis_data['dominant_dim'],
             step=step,
             # variance=var # <--- 移除这一行，因为 np.savez 的 kwargs 必须是存在的参数
            )
